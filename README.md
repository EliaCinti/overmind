...
| `OVERMIND_BACKUP_DIR` | Where `POST /api/backup` writes its archives (default `<data>/backups/`, created `0700`). The archive is the whole instance; the subscription token in it is sealed under the passphrase you give at export ([ADR-0044](docs/adr/0044-the-archive-is-the-instance.md)) |
