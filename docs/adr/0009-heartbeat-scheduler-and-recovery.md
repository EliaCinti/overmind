...
## Addendum — 2026-09-07: a wakeup takes the work the agent can take

Decision 5 said a queued wakeup "auto-starts the oldest `todo` task". Read literally, that made two things true that nobody wanted. First, a refusal the runner could name a remedy for (`RunnerError::Remediable`, the multimodal gate of ADR-0021) had no arm in `wakeup_outcome`, fell through as an error, and was propagated *before* the row was marked `done`: the wakeup stayed queued, the beat aborted — every later wakeup and the CEO's digests behind it — and the next beat refused the same task again. TravelAgency's log holds seventeen of them, thirty seconds apart. Second, and less visible: the oldest `todo` task is the one the picker always reaches first, so even with the refusal recorded, an agent that cannot take it would come back to it on every wakeup and never reach the work behind it — with nothing failing any more to say so.

So the contract is now: **a wakeup tries the tasks the agent is permitted, oldest first, within a window of twenty; a refusal that is the *task's* condition** (taken by someone else since the read, malformed for a start, refused with a remedy in hand) **is recorded and the next candidate is tried; a refusal that is the *agent's* condition** (over budget, paused, not characterized for the kind) **ends the wakeup at once**, because no other task would fare better. The outcome says what was declined before what was started, the remedies a refusal named ride in the `agent.wakeup_processed` row as data — the contract on `Remediable` is that nobody string-matches the sentence — and when the window was the limit, the outcome says so. The `match` on the start's result is exhaustive, no wildcard: the next variant added to `RunnerError` has to be decided in the scheduler, which is how this one should have been caught.

The window is a bound, not a policy: a beat is not the place to try a whole backlog. What ADR-0050 proposes — a cap on attempts and a repetition detector — is the general form of which this is the first instance.
