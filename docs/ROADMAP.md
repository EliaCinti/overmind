...
- **Accept:** agent completes a task in its worktree ✓, diff visible via `GET /sessions/{id}/diff` ✓, every step audited with the chain still verifying ✓ (integration tests, 2026-07-19; verified with the stub adapter — a paid smoke run with the real Claude Code CLI was the manual follow-up, **partly discharged 2026-08-07**: the live CLI runs authenticated inside M10's sandbox, honours `--model`, and reports cost. It found two ledger defects the stubs could not — see M10. Still to exercise live: a `code` task against a real repo, artifacts, chat and a meeting.)
