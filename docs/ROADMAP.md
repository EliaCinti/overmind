...
- ☐ **Slice 3 — the agent-facing ceiling.** [ADR-0015](adr/0015-agent-facing-memory-tools.md)'s other half: agents calling `recall` and `why` themselves, over **stdio** with `BRAIN_DIR` pointed at their company's brain, plus the per-runtime MCP config translation layer that ADR records.
  This slice was blocked for weeks on a prerequisite that does not exist. ADR-0024 and this line both claimed the work needed `wadachi serve-http`, a long-lived authenticated endpoint said to ship in Wadachi 0.14. **It was never built** — the Wadachi CLI offers `init`, `doctor`, `sleep`, `obsidian`, `export`, `restore`, and nothing else, and the MCP server is stdio only. The claim is withdrawn in [ADR-0026](adr/0026-change-awareness-across-concurrent-agents.md).
  It is also not needed. Measured 2026-08-12 against a **freshly provisioned** brain — the case that failed before Wadachi 0.14.1 — eight separate writer processes all committed (8/8, 0.07 s) and four separate reader processes each saw all eight memories immediately; a `recall` costs ~0.6 s, of which the embedding model load is 0.11 s. Agents are CLI processes that take MCP servers from config, so stdio is the shape they already want. Slice 3 is therefore cheaper than planned: config translation, no new server.
