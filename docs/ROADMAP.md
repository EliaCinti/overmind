...
- ☐ **Slice 3 — the agent-facing ceiling, and supervision if it earns it.** [ADR-0015](adr/0015-agent-facing-memory-tools.md)'s other half: agents calling `recall` and `why` themselves, which needs the per-company brain reachable as a long-lived authenticated endpoint (`wadachi serve-http`) plus the per-runtime MCP config translation layer that ADR records. The same endpoint is what would let Overmind supervise a brain process per company — deferred in [ADR-0024](adr/0024-managed-per-company-brain.md) in favour of M7's stdio pool, and worth revisiting only if pool churn shows up as latency.
