...
| A member sees their companies and only theirs; the company-scoped surface refuses non-members; the instance owner passes everywhere | membership as the filter — **organizational, not adversarial**: bare-id sub-resources are gated by authentication, not yet membership (M25 slice B), and every account on the instance is one the owner invited | `the_door.rs` — `members_see_their_companies_and_only_theirs` |
