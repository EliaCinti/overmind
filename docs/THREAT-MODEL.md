...
- **Anyone with the machine, still.** Since M24 the API answers to a
  ...
  never a surprise. An **unclaimed** instance used to be exactly as open as
  before M24 — "the boundary is the credential, and until one exists a fresh
  install must be able to claim itself". It is not, since
  [ADR-0045](adr/0045-the-install-is-one-script.md): it answers about its own
  door and takes a claim, and the claim costs a code only somebody at the
  machine can read.
...
| A caller without a session gets nothing but a liveness ping — **before an owner exists as well as after** | the wall on every `/api` route; sessions stored hashed; the same wall on the socket upgrade ([ADR-0032](adr/0032-authentication-the-boundary-moves-off-the-machine.md)) | `the_door.rs` — `a_claimed_instance_refuses_the_sessionless`, `a_real_session_enters_and_a_forged_one_does_not` |
| An instance nobody has claimed is not an open API. It answers about its own door and takes a claim, and nothing else answers anybody without a session — not founding a company (which starts a task, which runs an agent CLI on this machine), not choosing who pays, not opening the subscription sign-in, not minting an invite — and **not the live socket**, which is worse than a request because it is authorised once, at the upgrade, and never again ([ADR-0045](adr/0045-the-install-is-one-script.md)) | the wall no longer waves the whole `/api` through while `users` is empty; `/restore` is the one exception and only while the instance is unowned, so a stranger on a *claimed* box is refused before the upload rather than after it | `the_door.rs` — `an_unclaimed_instance_says_so_and_answers_nothing_else`, `the_owner_only_routes_are_shut_before_an_owner_exists`, `the_live_socket_refuses_a_stranger_on_an_unclaimed_instance` |
| The claim costs a setup code the server minted at first boot and kept for itself, and **signing up is not the way around it** — until ADR-0045 an unclaimed instance handed ownership to whoever signed up first, with neither an invite nor a code, and the product's own door did exactly that | minted when no owner exists and none is waiting, written to `<data>/setup-code` at `0600` and to the log once, compared in constant time, deleted by the claim that spends it; signup refuses outright while nobody owns the instance, because there is nobody who could have invited anyone | `the_door.rs` — `a_claim_without_the_setup_code_is_refused`, `signup_is_not_a_way_around_the_claim`, `the_first_user_owns_and_billing_is_the_owners` |
| A restore costs the same code, and the archive is refused **before a single byte reaches the disk** — an archive is the size of somebody's whole instance, and a stranger must not be able to make this machine write one | the code is demanded as the `archive` field opens, not after the upload, so a client that sends the archive first is refused without it | `restore.rs` — `a_restore_without_the_setup_code_is_refused` |
...
| A restore lands on an **empty** instance only — no owner, no company, no sign-in — and is therefore exactly as open as the claim it is: whoever can reach the port of an unclaimed box can already claim it, and a restore is a claim with a payload. Once any of the three exists, the door answers a stranger and the reason is the owner's to read. Emptiness is not a check made once and trusted — a legitimate claim landing while a slower restore is still unpacking, or between that restore's answer and the next boot, must still win: `stage()` re-checks right before it commits, a landed claim clears any restore staged before it, and the boot swap opens the *live* database and refuses the swap outright if it is no longer empty (or cannot be read). An archive is checked whole before anything moves — every entry against the manifest's hash, no entry the manifest does not name, no path that climbs out of the tree, nothing that is not a plain file, the audit chain verified *and* equal to the report the manifest carries, its KDF parameters bounded so a manifest cannot hand the token's unsealing a memory allocation large enough to abort the process — and a wrong passphrase refuses the whole restore while a retry is still free ([ADR-0044](adr/0044-the-archive-is-the-instance.md)) | the emptiness predicate re-checked at three points (the route, immediately before `stage()` commits, and live on the database immediately before `swap_pending`'s destructive delete), `auth::claim` clearing a stale restore the instant it lands, `unpack_checked` against the manifest, `audit::verify` on the staged database, a bound on `unseal`'s KDF parameters, and a staging tree deleted on any refusal | `restore.rs` — `a_restore_lands_only_on_an_empty_instance`, `a_tampered_archive_is_refused_by_name_and_nothing_is_staged`, `an_entry_the_manifest_does_not_name_is_refused`, `a_broken_chain_is_refused_even_when_every_hash_matches`, `a_wrong_passphrase_refuses_the_whole_restore_while_a_retry_is_free`, `a_claim_that_lands_after_a_restore_was_staged_wins_at_the_next_boot`, `a_claim_clears_a_restore_staged_earlier_the_instant_it_lands`, `a_manifest_asking_the_kdf_for_a_terabyte_is_refused_before_it_is_spent` |
...
| An agent that skips the adapter's permission prompts is always one the OS is already holding | one predicate, `sandbox::caged`, asked of the whole set of mechanisms rather than of any one — and, since 6 Sep 2026, **asked once**: a run builds a single `Confinement` and reads it both for the flag and for the cage it applies. Asking twice was the hole. Each call built its own Landlock ruleset against a filesystem that can change between them, and a failed build degrades to `None` with a log line, so the first answer could be *yes* — which is what grants `--dangerously-skip-permissions` — while the second produced no Landlock at all. Reachable on a Linux host outside the image, where Landlock is all `is_real` has to go on ([ADR-0023](adr/0023-os-level-sandboxing.md) addendum, [ADR-0029](adr/0029-the-cage-inside-the-container.md)) | `runner.rs` — `the_permission_flag_never_travels_without_the_cage`; the single build is structural: `command` now takes the `Confinement`, so a second one cannot be built without changing its signature |
| A credential the sign-in scraped never reaches a log, an error, or the interface | `scrub_secrets` over the **whole** transcript before it is truncated, and a redaction that follows a token across the line breaks `strip_ansi` leaves behind. Both halves were holes until 6 Sep 2026: every site read `scrub_secrets(&tail(output, N))`, so a prefix pushed out of the window left the body unrecognised and printed; and a TUI redraw's carriage return became a newline mid-token, splitting it into a short run carrying the marker and a long run carrying the secret, neither of which the scrubber recognised — rejoining the two lines gave the credential back | `claude_auth.rs` — `a_prefix_cut_off_by_the_tail_still_hides_the_body`, `a_token_split_by_a_redraw_is_still_a_token`, `ordinary_output_is_left_alone`, `a_credential_is_scrubbed_from_outbound_text` |
| An agent cannot end the server from inside its own run directory | the hand-over that chowns a run directory walks it iteratively with a depth cap, not by recursion. Recursion descended once per directory level inside `spawn_blocking`'s 2 MiB stack, over a tree the *previous* run wrote — so a deep enough nest made the next hand-over overflow and abort the process | `sandbox.rs` — `a_tree_too_deep_to_hand_over_is_refused_rather_than_overflowing` |
