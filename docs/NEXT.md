...
**Designed in [ADR-0048](adr/0048-a-provider-is-a-capability-not-a-name.md), 4 Sep 2026.** The survey there supersedes the estimate below in two places: the seam is eight members rather than six, and the hard part is not the interface — it is that a provider which cannot report what a turn cost makes the budget cap silently stop applying, because `runner.rs` treats a missing cost as ordinary.
