...
### The sign-in and the cage, before they move *(read first)*
*A `/code-review medium` aimed deliberately at what ADR-0048 has **not** yet touched — `claude_auth.rs` and `sandbox.rs` as they stand — asked for on 5 Sep 2026 because the three slices done so far turned up six real pre-existing bugs each time the boundary made two distant pieces of code adjacent. Fifteen findings. I verified the five below in the code myself rather than relaying them.*

**Five that should not wait for the extraction.**

1. **A retry can hang the server, permanently.** `start()` holds the global `FLOW` lock while it kills a stale child and installs a new flow. The *old* reader thread is blocked on that same lock; when it wakes it calls `slot.as_mut()` — which now hands it the **new** flow — and does `child.wait()` on a live process **while holding the mutex**. Every later `status`, `submit_code` and `start` blocks forever, and since the axum handlers call them synchronously, tokio workers go with them. The path is the one the stale branch was written for: a flow stuck exchanging for two minutes, and the person clicks *Retry*. There is no flow id, so the epilogue cannot tell whose flow it is holding.

2. **A credential can reach the log in the clear, in the branch written to prevent that.** All four sites read `scrub_secrets(&tail(&f.output, N))` — the truncation happens *inside*, so the scrubber only ever sees the last N characters. `scrub_secrets` recognises a secret by the literal `sk-ant`, so when the prefix falls outside the window and the body does not, 40-odd characters of a live token pass through untouched into the `Failed` state, the UI tail, and `eprintln!`. Scrub the whole transcript, then truncate.

3. **And `strip_ansi` can split a token so the scrubber never sees it whole.** It turns `\r` into `\n`; `find_token` reads the raw bytes for exactly that reason, but `scrub_secrets` reads the stripped text and scans runs of `[A-Za-z0-9_-]`. A carriage return mid-token yields a short prefix run with no `sk-ant` and a long body run with no `sk-ant`: both printed as-is, and rejoining the two lines gives the credential back.

4. **Choosing to pay by key does not stop the plan from being billed.** `keep_key_away` is gated on `plan_is_preferred`; `inject_oauth_token` is gated on nothing. Somebody who signed in once and later chose *detected* still gets `CLAUDE_CODE_OAUTH_TOKEN` in every agent environment. The asymmetry is verified in the code; that the token then outranks `ANTHROPIC_API_KEY` in the CLI is the reviewer's claim and wants a measured run before it is stated as fact.

5. **An agent can kill the server from inside the cage.** `chown_tree` recurses once per directory level with no depth cap, inside `spawn_blocking`'s 2 MiB stack, over a tree the previous run wrote. A deep enough directory makes the *next* hand-over overflow and abort the process. An explicit worklist with a depth limit costs nothing.

**And one the cage's own promise depends on:** `caged()` builds a Landlock ruleset to answer a boolean and throws it away; `command()` then builds a second one against a filesystem that may have changed. `caged()` returning true is what grants `--dangerously-skip-permissions`, so a build that succeeds in the predicate and fails in the command leaves an **uncaged** run holding the caged flag. Build the `Confinement` once and pass it to both.

**Six more that travel with `sign_in` when it moves**, and are the reason to move it rather than copy it: the `agent_cmd` guard makes sign-in structurally unreachable for a second provider (provider B *is* configured that way); `token_path` is one fixed filename with no provider component, so B's token overwrites A's, and `backup.rs` has the same fixed shape; `find_token` anchors on `sk-ant-` and `scrub_secrets` on `sk-ant`, so another vendor's credential is neither scraped **nor redacted** — the redaction half being the dangerous one; `clock_skew_secs` hardcodes `api.anthropic.com`; `adapter_paths()` reads the *server's* `HOME`, granting `/root/.claude` in the image where the agent's home is `/home/agent`; and `GET /api/claude-auth` re-runs the economy probe and overwrites shared state on every poll, which only goes unnoticed because the React client stops polling.

Also: a failed respawn leaves the killed flow in the slot advertising a dead URL; a 300 ms blocking sleep is held across the global lock on a tokio worker; the ten-minute lifetime is enforced only if somebody calls `start()` again, so an abandoned flow keeps its child, pty and thread until the server exits; `stored_token` swallows every read error, so an unreadable token file is indistinguishable from never having signed in; and `sandbox.rs:421`'s doc comment belongs to `command()`, not `inject_oauth_token`.
