...
pub(crate) async fn drain_narrating(
    ...
    // Bound once, outside the loop: the provider is a `&'static dyn`, so this
    // costs nothing and keeps the hot path free of a lookup per line.
    ...
    let out_task = tokio::spawn(async move {
        ...
        if let Some(stdout) = stdout {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                if let Some(delta) = provider.text_delta(&line) {
                    if accum.len() < 64_000 {
                ...
                } else if let Some(activity) = provider.activity(&line) {
...
async fn finalize(ctx: &SessionContext, outcome: Outcome) -> Result<(), RunnerError> {
    ...
    if let Some(adapter_sid) = crate::provider::current().session_id(&f.output) {
    ...
    if let Some(cost) = crate::provider::current().cost(&f.output) {
