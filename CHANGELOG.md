...
- **The installer lands where you are, and each install is its own instance.** `curl … | sh` now makes `./overmind` beside you instead of `$HOME/overmind` — reported by the owner, who ran it from his Desktop and found nothing there. A subfolder rather than the bare working directory, so nothing is scattered into whatever directory you happened to be in; `OVERMIND_HOME` still overrides.
  That change alone would have been worse than the problem it fixes. `docker-compose.yml` carries no `name:` because *Compose takes the project from the directory, so two folders are two instances* — true only while the folders have **different names**, and an installer that always makes one called `overmind` hands every install the same project, container and port. The second `docker compose up -d` would not stand an instance beside the first: it would adopt its project and recreate its container onto the new folder's empty `./data`. `$HOME/overmind` had been safe by accident, because exactly one such folder ever existed.
  So the installer writes a `.env` beside the compose file — `COMPOSE_PROJECT_NAME`, `OVERMIND_NAME` and `OVERMIND_PORT`, derived from the folder's full path — once, and never again, so re-running cannot rename a project out from under a running container. A folder that predates the file keeps the name Compose was already using for it, which is what stops an update from orphaning your instance. The published port is now `${OVERMIND_PORT:-7070}`: the first install still gets 7070, a second gets one of its own rather than "the first one free", which hands the same number to two installs made while both were stopped.
- **And it looks for what you already have, instead of forbidding what you might break.** With the collision gone there is nothing to refuse, so it does not refuse — it asks Docker. Every container running Overmind's image names its install directory exactly through its `/data` bind mount, so an instance is found wherever it lives, whatever its folder is called, running or stopped; a folder merely *called* `overmind` with nothing in it is correctly not one. When one is found and this is not it, you are told where it is and offered the choice: update that one, make a new one here, or stop. Asked on `/dev/tty` — piped to `sh`, the script's own stdin is the pipe — and only where a terminal exists, so CI is never asked.
### Fixed
...
- **The CEO stops reporting starts it was never told the outcome of** ([ADR-0046](docs/adr/0046-a-start-the-ceo-cannot-see.md), M33). Measured on a real company: three times in one conversation the CEO wrote *"now I have put three in run"* and the board stayed at **zero in progress** — and by the third time it had begun apologising for the wrong thing, guessing it had described starts instead of executing them. It had executed them. The reply and its `start` list are one JSON object: the words are committed, and only then does the server try the starts, so the CEO cannot have seen what happened. `start_existing` already reported it — no task by that title, nobody assigned, or which of the three autonomy outcomes — and `ceo.rs` threw all of it away, matching only on `Err` and writing that to stderr. One sentence covered five outcomes.
  Now the server says what became of each start, in the company's language, as a `system` line in the same thread — not by editing the CEO's words, which are already delivered by then. A start that simply ran adds nothing; the board shows it. The rest say what happened: no open task carries that title, the task has nobody on it, the start is in your inbox waiting for you, whoever holds it only proposes so a person must start it — or the budget refused it, **with both amounts and the remedy**. `RunnerError::OverBudget` was a bare variant naming neither the cap nor the spend; it carries both now.
  On the company where this was found, the gate had refused six starts in two hours because one agent had spent €50.38 against a €50.00 cap. The gate was right every time. Its silence was the bug.
